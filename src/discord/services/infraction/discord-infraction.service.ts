...
export class DiscordInfractionService {
  ...
  private async handleAddInfraction(
    ...
    try {
      ...
      const maxInfractionPoints = Math.max(
        ...config.infractions.map((inf) => inf.points),
      );
      ...
      if (roleOption?.value) {
        await member.roles.add(
          roleOption.value,
          `Sanción: ${reasonOption.value}`,
        );
      }

      await this.userDiscordService.addPenaltyPoints(
        user.id,
        infractionConfig.points,
      );
