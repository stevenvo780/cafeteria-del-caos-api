...
export const buildCommandsList = async () => {
  ...
  for (const config of Object.values(DISCORD_COMMANDS)) {
    for (const commandData of Object.values(config)) {
      const options = typeof commandData.options === 'function' ? await commandData.options() : commandData.options || []
      commands.push({
...
export const getCommandCategory = (commandName: string): CommandCategories | null => {
